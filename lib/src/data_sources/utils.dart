...
class Utils {
  ...
  static void prepareLogDataDoingTest({
    required LogModel? log,
    required Map<String, dynamic>? data,
    required String? message,
    required String status,
    ...
    addLogDoingTest(log, status);
  }

  static void addLogDoingTest(LogModel log, String status) {
    if (status != "none") {
      //NOT Action log
      DateTime createdTime =
      DateTime.fromMillisecondsSinceEpoch(log.createdTime);
    ...
    //Convert log into string before write into file
    String logString = convertLogModelToJson(log);
    writeLogDoingTestIntoFile(logString);
  }

  static Future<List<String>> readLineFile(File file) async {
  ...
  static Future<void> writeFile(File file, List<String> lines) async {
    await file.writeAsString(lines.join('\n'));
  }

  static void writeLogDoingTestIntoFile(String logString) async {
    String folderPath = await FileStorageHelper.getExternalDocumentPath();
    String path = "$folderPath/flutter_doing_test_logs.txt";
    if (kDebugMode) {
      print("DEBUG: log file path = $path");
    }
    File file;
    ...
    List<String> lines = await readLineFile(file);
    if (lines.length >= 1000) {
      lines.removeRange(0, lines.length - 999);
    }
  ...
  static void sendLog() async {
    if (Platform.isIOS) {
      ...
      channel.invokeMethod(sendLogsTask, {
