...
class _TestRoomWidgetState extends State<TestRoomWidget>
  ...
  AudioRecorder? _recordController;
  ...
  Future<void> _initRecordController() async {
    ...
    try {
      _recordController = AudioRecorder();
  ...
  Future<void> _stopRecord() async {
    if (null != _recordController) {
      ...
      // Map<String, dynamic> info = {
      //   StringConstants.k_question_content: _simulatorTestProvider!.currentQuestion.content,
      //   StringConstants.k_file_path: _simulatorTestProvider!.currentQuestion.answers.first.url
      // };
      //
      // _createLogDoingTest(action: LogEvent.stop_audio_record, data: info);
    }
  }
  ...
  Future<void> _startRecord(
    ...
    try {
      await _recordController!.start(
        RecordConfig(
            encoder: Platform.isAndroid ? AudioEncoder.wav : AudioEncoder.pcm16bits,
          bitRate: 128000,
          sampleRate: 16000
        ),
        path: path
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
        DiagnosticsProperty<AudioRecorder>('_recordController', _recordController));
  }
